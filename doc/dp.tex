...
was actually a macro, and replaced each occurrence of the formal with
passed-in actual.
...
In context of GENERIC, block is expressed with an expression node
\literal{BIND\_EXPR}.  Block expression has three operands: a chain of
variables local to the block, a list of statements to process, and
associated block.
...
$A_B$ appears as a statement inside a statement $B_S$ of bind
...
\literal{LOOP\_EXPR} endlessly processed.  Computations of loop
control variables have to be done explicitly.
...
for OpenMP, a parallel language, an area that is about to become {\em
  very} interesting in years to come.  It can handle classes, and
