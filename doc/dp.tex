...
\edeclaration{ I hereby declare that this work has been created by me,
  ...
  supervision of Ing. Luká¹ Szemla.  All information resources that
  I have used are properly cited.  }

\acknowledgements{ I owe thanks for patience and advices to the
  thesis' supervisor, Ing. Luká¹ Szemla, who was bombarded by
...
\fitabstract{
The entry barrier to the development for GCC got considerably lower
during the last years.  Articles with various architectural overviews
and how-to documents pop up in magazines, websites, and on conferences.
With official intermediate language, GENERIC, used for communication
between front end and the rest of the compiler, things are yet easier:
it's no more necessary to bear the tedium of RTL when one writes new
front end.

Yet there is a complexity inherent in handling a source base the size
of GCC.  There are files to be written, peculiar options to be set up,
and this all with relatively thin documentation.

This work is written to help with this last point.  An example front
end is described, with everything from the source base setup, through
various GENERIC constructs, up to the issues of employing in-line
assembly, using GCC native preprocessor, or preparing the front end
...
%ahilmorsxyz9};>
%áèéìíïòóøùú¹ý»¾
Vstupní bariéra pro vývoj uvnitø GCC se bìhem posledních let znatelnì
sní¾ila.  Na konferencích, v~èasopisech a~na webu se objevují èlánky
s~architektonickými pøehledy a~návody.  Vìci se nadále zjednodu¹ují
pou¾itím oficiálního vnitøního jazyka GENERIC: pro komunikaci mezi
pøední èástí a~zbytkem pøekladaèe ji¾ není nutné zabývat se obtí¾ným
a~nepøehledným RTL.
...
Cílem této práce je pomoci s posledním zmínìným bodem.  Práce popisuje
ukázkovou pøední èást: v¹e od vytvoøení zdrojových souborù, pøes rùzné
konstrukce jazyka GENERIC, a¾ k problémùm se zapojením assembleru,
pou¾ívání nativního preprocesoru, nebo pøípravì pøední èásti pro
kroskompilaci.
}
...
several C modules with AST and semantic analysis.
...
undefined structure.  Only in the module that handles that particular
type is the structure actually defined, so that the implementation has
full, unrestricted access to bits.

The encapsulation zeal is exposed by use of visitors.  There are
certain polymorphic structures in \Algol AST.  For example expressions
may be numbers, variable references, unary and binary expressions,
function calls, etc.  All expression kinds are passed around as the
same opaque \variable{expression\_t} pointer.  To do any action
depending on exact type of expression, one has to define a {\em
  visitor} over the expression structure, and let the visitor dispatch
on the desired expression object.

Other means of achieving safety include strong trend towards static.
Whatever can be checked by compiler giving a warning is checked so.
If the warning can be turned to full error with some extra coding, so
much the better.  If compiler can't check that, runtime will.
...
\function{a60\_expr\_callback} in this case.  Callback builder makes
sure that your callback function takes \literal{expression\_t*} as a
first argument.  All is checked during compilation: that you didn't
forget one of the callbacks, that all callbacks are typed right, etc.

In addition, the visitor builder knows it builds visitor for
expression type, and when in debug mode, it {\em dynamically} checks
that that visitor is dispatched over expression, and not
e.g. statement.  In release mode, the check goes away.
...
doesn't match, aborts.  Thus you can't conditionalize on the result of
typecheck.  The mechanism is here to {\em prevent errors}, not to
emulate dynamic typing systems.  This is C, you have to know what
pointers you're passing around!
...
an object (passed in as \literal{void*}), it can decided whether the
...
that, given a \literal{void*} pointer, can decided whether the given
...
goes completely away, so there is no performance cost.
...
determined by those of some internal \literal{enum} values.  Signature
is \literal{char const*}, and is used as follows:
...
This is extremely useful for debugging and diagnostic messages. All
you have to do to find out the type of mysterious object is to
dereference the pointer and interpret it as a string.

When creating a visitor, not a signature, but address of signature
variable is used.  When doing a dispatch, the visitor performs simple
check:
...
As you can see, this very typing mechanism gives us ability to deduce
what's wrong, and inform a user in meaningful way:
...
provided on connecting such minimal frontend with preexisting language
parser, and on versioning GCC component apart from GCC itself.

%more items to come in final thesis
...
question over an email, or source code.  This very synthesis is sole
contribution of the author: no other work exists, that would provide
the detailed and continuous explanation on the topic.
