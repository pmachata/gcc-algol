...
assembly.  This thesis also contains notes on preprocessing, emitting
debuginfo, or integrating runtime library.
\fi
...
are not directly related to the translation of source language.  These
include use of preprocessor (\ref{Preprocessing}), processing command
line options (\ref{CommandlineOptions}), emitting diagnostic messages
...
However the name will be used in makefile targets, and users will call
...
Note that the fragment actually doesn't present hooks, but Makefile
rules.  Only the first rule in the list is actually a hook.  The first
two rules, then, have not only the dependence list, but also a body.
This doesn't hold for the other two rules, which only contain
dependence list, and use mechanism of implicit rules to sort out the
exact order of compilation.

Because other hooks are empty, including \variable{uberlang.install-*}
family, the minimal frontend won't be installable.  You will have to
run it like this:
...
off already written codebase.  On the other hand, if you know you are
...
\chapter{The \Algol 60 GCC Frontend}
\label{Algol60GCCFE}
...
The programming language \Algol 60 was picked purposefuly, because
...
I strived to avoid any extra dependencies of the front end, to the end
...
do any actual work, it just typecasts to pointer, but as an argument,
it requires a function pointer that takes \literal{expression\_t*} as
a first argument.  If your callback function doesn't honor this,
compiler will give you a warning.  Visitor builder, in turn, requires
exact number of callback arguments.
...
doesn't match, aborts.  You can't conditionalize on the result of
typecheck, the mechanism is here to {\em prevent errors}, not to
...
The \Algol 60 spec groups together two notion: numerical expressions
...
compiler, these two kinds are disjunct: computed, numerical
...
designational expressions are completely disjunct polymorphic
hierarchy, used in other context, namely as tagets of the
...
\ind\=\keyw{switch} b := q, r, 5;\\
    ...
    \>\keyw{integer} \keyw{array} ar; \keyw{integer} ix, k;\\
    \>initialize(ar, ix, k);\\
    \>\keyw{goto} a[ar[ix]];\\
q:  \>puts(\litstr{`q!'});\\
r:  \>puts(\litstr{`r!'});\\
...
The example also shows that it's possible to use both numberical and
...
these days.  Supported leaf statements are assignment, goto,
conditional, and for.  Most of them work as expected.  Assignment
statement supports assignment of one value to several variables
(without reevaluation).  Goto has unusually rich syntax for
computation of target, see the description of designational
expressions in previous section.  For has also rich syntax for
iteration, see this example:
...
The compiler uses auxiliary polymorphic type \keyw{for\_elmt\_t} to
...
and booleans, and one compound type: an array of basic types.  Number
of dimensions of array is static property of the declaration, but the
dimensions themselves are computed.  Each variable can be marked
``own'', which is basically the same as \literal{static} in C.  The
last supported type is string, which is only allowed as procedure
...
The declaration on its own right is a procedure.  Procedures are {\em
  the} reason why the \Algol 60 is kinky.  In particular, formal
argument of procedure can be declared as {\em pass by name}, which
basically means that the actual argument of this formal parameter is
reevaluated each time it's hit in control flow---like if the function
was actually a macro, and replaced each occurence of the formal with
passed-in actual.
...
integer, or integer function without parameter.  Depending on call
site in effect, and the way each identifier end up resolved, compiler
...
of signature itself.  Before a dispatch, the visitor performs a simple
check:
...
Messages in the compiler are formatted in usual unixish format:
...
dynamic typechecking, is heap-allocated, and opaque.

Each message is output to log with given {\em severity}, which can be
one of {\em debug}, {\em info}, {\em warning}, {\em error} or {\em
  fatal error}.  Each log has associated a filter treshold: messages
with severity below the treshold are thrown away, the rest is written.
...
(which is a wholesome thing to do, because it will discover certain
errors even if the function is not called), and it fails to resolve
variable reference in the body itself, it marks that variable as an
implicit parameter of the function.  Such variable is then assigned a
placeholder type of \literal{implicit}.

The last mentioned was type \literal{unknown}.  This type is assigned
to expression whose type is not known, most often because there was an
error found in the expression.  When the resolver gets this type, it
will explicitly not emit any error messages.  This is done at the leaf
expression that caused the type to be \literal{unknown} in the first
place.
...
how is the compilation done will be described in the chapter.
...
Translation of \Algol 60 AST to GCC GENERIC is task of the \Algol
Binder, the last pass in compilation process.

This chapter describes how to generate GCC intermediate language
called GENERIC, and uses a lot of examples from the \Algol Binder.  It
is organized as a catalogue of typical constructs found in programming
language, and typical tasks performed by a compiler, with explanation
how to handle these in context of GCC.

In comparison with GCC internals (\cite{TR:GCCInt}), the description
in this thesis is kept in more abstract level.  I consider GCC to be a
target of compilation process, and as such am more interested in how
the nodes are created and how to express various imperative
constructs.  GCC internals documenatation is directed also towards the
analysis of GENERIC.
...
\program{\ind\xtree ret = \keyw{NULL\_TREE};}

\xtree is itself a pointer type, so \literal{\xtree *} denotes a {\em
pointer to \xtree}.  The value \literal{\keyw{NULL\_TREE}} is thus
analogy to \literal{\keyw{NULL}} pointer of the \literal{\keyw{void}*}
world.  Pointer to \xtree is ocassionally also used, e.g. as an output
parameter of a function.

Each node has an arity, a number of child nodes that it needs; and
{\em signature}\footnote{Which is an informal term, not used by GCC
people as far as I know.}, which describes what type each of children
nodes has to have.
...
emitting wrong code, or failed with SIGSEGV.)  There is little I can
advise, generally just read \file{tree.def}, read \cite{TR:GCCInt},
and look to other front ends, in this order.  Often it is helpful to
inspect the failing site.  Surprisingly often it's possible to deduce
what went wrong by looking at the assertion or comments nearby.
...
the same name.  Identifier nodes have the property that they are
unique: there is only one identifier node ever made for any particular
name.
...
most of the type building procedures thus look like this:
...
support 128-bit floating point, e.g. ia64 \cite{TR:Itanium}.  GCC has
ways to deal with this diversity.  It allows construction of floating
point literals from string as well as from various represenatation,
has dedicated funtions for comparison and formatting them, and can
also convert internal floating point number back to some target
representation.  Look up the \file{gcc/gcc/real.h} header if you are
interested.
...
created with \function{build\_complex}.  The arguments are simply a
type, a real node, and an imaginary node.
...
of writing debuginfo. The contents of locus are a {\em name of the
  file} and a {\em number of the line} inside that file, where given
construct appeared.  Thus its creation is straightforward, one just
has to include the right headers.  E.g. conversion between \Algol 60
front end's {\em cursor} type and locus is done this way:
...
\\
    \>\keyw{void}\\
    \>cursor\_to\_loc (cursor\_t \keyw{const} * cursor, location\_t * loc) \{\\
...
the beginning, followed by a list of options separated by whitespace,
...
Let's have a look at the non-preprcessing spec string.  There is only
...
\file{gcc/gcc/gcc.c}), but the magic that gives you the precompiler
...
GCC uses its own functinos for reporting problems.  You may or may not
...
is GCC's own testsuite, you will have to provide means of converting
from your format to what's expected by the testsuite scripts.  Third,
...
are just historical crud that you use only inadverently.  Anyway, GCC
...
development tools, \file{autogen} in
particular\footnote{\literal{autogen} has a surprising dependency of
its own: \file{guile}.}.  Build system in release version of GCC will
...
have to be handrolled or cut'n'pasted.  As far as I know, this is the
...
testsuite is an absolute necessity.  The GCC project does contain such
testsuite.  It's built on popular \literal{dejagnu} test harness, and
...
This works fine for compilers: the testsuite contains source files to
...
If you want to include other means of testing, e.g. the testsuite that
is already written as part of your front end, do so by explicitly
...
no sense.  Setup of testsuite scripts for your language is a
convoluted process, and I recommend simply copying over testsuite
...
directives, a metainformation that describes what to do with the test
...
\ind\=\keyw{integer} a;\\
    ...
    \>b := \litstr{`ahoj'}; \keyw{comment} \comment{\{ dg-error "type mismatch" \}} ;\\
\keyw{end};}
...
verbose, or differently formatted messages, you have to rewrap them to
suit expectations.  Have a look at
...
\item A minimal frontend and its integration into GCC build system is
  described in this work.  All files that the frontend is comprised of
  are thoroughly examined, and pitfalls mentioned.  Several tips are
  provided on connecting such minimal frontend with preexisting
  language parser, and on versioning GCC component apart from GCC
  itself.
...
that would provide the detailed and continuous explanation on the
topic.

\section{Future Research}

This thesis describes odds and ends of the GCC front end to middle end
interface.  If there is a future research associated with this area,
it's most probably more thorough description.  GCC has native support
for OpenMP, a paralelization language, an area that is about to become
{\em very} interesting in years to come.  It can handle classes, and
object oriented programming in general.  This thesis does not describe
these topics.

\iffalse
Embedded system developers would certainly use a cross compilation and
bootstrapping manual: is it possible to write parts of \Algol 60 front
end, or its runtime library in \Algol 60 itself?  (The answer is yes,
but the question is ``what does it take?'')  What does a developer
need to do to prepare his or her compiler for cross compilation?
\fi

In general, GCC is very attractive target for a lot of
compiler-related research.  It's open development cycle makes up for
great test bed: be it a new radical systems language, be it new
optimization technique, new hardware platform in development, GCC
...
\item GCC 4.2.x with resolved dependencies.  GCC 4.1.x or earlier will
  ...
  (e.g. \file{gcc-4.2.0.tar.bz2}), or core package and testsuite
...
testsuite passed on systems with following triplets:
...
case.  This notion is used also for assembly and fortran source
files).  For example, let's have the two files as shown on figures
...
Most of this work is done by GCC itself: GCC launches preprocessor,
decides that the processing should be stopped after that when
\option{-E} is passed in, and GCC generated the make dependency rule.
