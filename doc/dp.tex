...
different data types may occupy the same memory location.  Each
...
Leaves would be identifier references, integer numbers, etc.  Internal
nodes would be unary or binary operations, block containers, etc.
...
own node GENERIC types.  Of course it wouldn't know how to gimplify
...
\item[$\bullet$] The language that is used to represent programs in
  ...
  C-like tree language.  Apart from predefined node types, front end
...
\item[$\bullet$] While it is possible to use GENERIC as an AST
  representation, it is advisable to use rather as a target language,
  and craft AST to suit your front end's needs.
