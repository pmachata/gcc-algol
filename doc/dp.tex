...
communication isn't done through option strings, instead, \GCC
...
file gives, though build magic, rise to \file{options.h}.  Format and
...
dynamic typing (look at OBJ\_TYPE\_REF),
and others to GENERIC
(with the help of proper hooks).  Show examples taken from \Algol 60
code.

Elaborate on integer support in \GCC.  How to support big integers,
what's necessary big/little endian-wise, if there's anything to know
about bits-per-byte, etc.  Similarly for floats.  I.e. cover
everything from signed char to unsigned {long} int, and from single to
{long} double :).
